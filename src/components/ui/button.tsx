import * as React from "react"
import { Slot } from "@radix-ui/react-slot"
import { cva, type VariantProps } from "class-variance-authority"

import { cn } from "@/lib/utils"

const buttonVariants = cva(
  [
    "inline-flex items-center justify-center gap-2 whitespace-nowrap",
    "rounded-[12px] text-sm font-semibold",
    "ring-offset-bg-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
    "disabled:pointer-events-none disabled:opacity-50",
    "transition-[box-shadow,transform] duration-200 ease-in-out",
    "[&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  ].join(" "),
  {
    variants: {
      variant: {
        default: [
          "bg-primary text-primary-foreground shadow-small",
          "hover:shadow-medium hover:-translate-y-px",
        ].join(" "),
        destructive: [
          "bg-destructive text-destructive-foreground shadow-small",
          "hover:shadow-medium hover:-translate-y-px",
        ].join(" "),
        outline: [
          "border bg-bg-light text-text-primary shadow-small",
          "hover:shadow-medium hover:-translate-y-px",
        ].join(" "),
        secondary: [
          "bg-bg-light text-text-primary shadow-small",
          "hover:shadow-medium hover:-translate-y-px",
        ].join(" "),
        ghost: [
          "text-text-primary",
          "hover:bg-bg-light",
        ].join(" "),
        link: "text-primary underline-offset-4 hover:underline",
      },
      size: {
        default: "h-10 px-5 py-2",
        sm: "h-9 px-4",
        lg: "h-11 px-8",
        icon: "h-10 w-10",
      },
    },
    defaultVariants: {
      variant: "default",
      size: "default",
    },
  }
)

export interface ButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>,
    VariantProps<typeof buttonVariants> {
  asChild?: boolean
}

const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : "button"
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, className }))}
        ref={ref}
        {...props}
      />
    )
  }
)
Button.displayName = "Button"

export { Button, buttonVariants }
